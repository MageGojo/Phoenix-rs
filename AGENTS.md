...
4. 选库时同步 `config/database.toml` 与应用 `Cargo.toml` 的数据库 feature（`sqlite` / `pgsql` / `mysql`，且 `default = []` 时需显式启用）；其它能力（`tls` / `websocket` / `sse` / `auth` / `jwt` / `password` / `metrics` 等）按需开 feature，见 `docs/FEATURES.md`。密钥放 `.env`。
