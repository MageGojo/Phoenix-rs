...
推荐的外到内顺序是：`TrustedProxies` → `RequestId` → `AccessLog` → `HostAllowlist` → `Cors` → `RateLimit` → `SessionMiddleware` → `Csrf` → `NonceSecurityPolicy` → 业务 Handler。`Routes::with_middleware` 的首次注册层位于最外侧；改变 Session/CSRF、request ID/日志或 nonce/Page 的相对顺序会破坏相应上下文。
...
中间件生成 128-bit 随机 nonce 并写入 Request extensions。Handler 输出转换保留一个不打印 Header 值的 `ResponseContext`，所以直接返回 `Page`、`Result<Page, _>` 或状态码 tuple 也会自动继承 nonce。相同值会出现在：
...
Nonce 不属于 `PageEnvelope`，不会进入页面 props、局部导航 JSON、contract hash 或共享业务缓存。包含 nonce 的 HTML 固定覆盖为 `Cache-Control: private, no-store`，并移除 `ETag`/`Last-Modified`；JSON/API 响应的缓存策略不因 nonce 中间件改变。不要设置 Vite `html.cspNonce`，因为那会把构建期常量误用为跨请求 nonce。

静态 CSP 进入 nonce 模式前会拒绝重复 directive、`'unsafe-inline'`、预置 `'nonce-*'` 和控制字符。嵌套相同 nonce policy 会复用 Request 中的值；不同策略产生的下游 CSP 冲突返回通用 500。第三方 script/style source 仍必须由应用在受审查的 `SecurityPolicy` 基础上显式允许。只有确认所有子域都支持 HTTPS 后才启用 HSTS `includeSubDomains`，HSTS 本身不能替代 TLS 终止。
