...
- 默认脚手架按环境安装请求级 CSP nonce、`X-Content-Type-Options`、Referrer Policy 等安全头；开发模式只信任显式 Vite origin。
...
- `SecurityPolicy` 提供静态 CSP/HSTS 基线；`NonceSecurityPolicy` 在此基础上生成请求级 nonce，并自动连接 HTML、Vite 与 renderer。显式下游 CSP 与计算结果不一致时返回通用 500。
...
- `BlindIndexer` 提供带用途隔离、版本化 envelope 和有界 key rotation 的 HMAC-SHA256 等值查询索引；它是确定性认证标签，不是加密。

这些能力由各 crate 的单元测试与 `examples/blog/tests/` 中的真实 TCP/进程内集成测试覆盖。

推荐的外到内顺序是：`TrustedProxies` → `RequestId` → `AccessLog` → `NonceSecurityPolicy` → `HostAllowlist` → `Cors` → `RateLimit` → `SessionMiddleware` → `Csrf` → 业务 Handler。`Routes::with_middleware` 的首次注册层位于最外侧；nonce 层必须位于可能短路的 Host、限流、Session 和 CSRF 层之外，才能让 4xx/5xx 同样携带 CSP、HSTS 与浏览器安全头。它仍位于 `TrustedProxies` 之后，以便基于可信有效协议决定 HSTS。改变 Session/CSRF、request ID/日志或 nonce/Page 的相对顺序会破坏相应上下文。
...
生产页面使用 `NonceSecurityPolicy::default()`。开发环境必须把 `VITE_DEV_URL` 解析为一个不带凭据、路径、query 或换行的精确 HTTP(S) origin；框架只为该 origin 推导对应 HTTP 与 WebSocket source：

```rust
let policy = if cfg!(debug_assertions) {
    let vite_origin = std::env::var("VITE_DEV_URL")
        .unwrap_or_else(|_| "http://127.0.0.1:5173".to_owned());
    NonceSecurityPolicy::development(&vite_origin)?
} else {
    NonceSecurityPolicy::default()
};
```

中间件生成 128-bit 随机 nonce 并写入 Request extensions。Handler 输出转换使用不复制敏感 Header、Debug 不输出 query 的 `ResponseContext`，所以直接返回 `Page`、`Result<Page, _>` 或状态码 tuple 也会自动继承 nonce。相同值会出现在：

- `Content-Security-Policy` 的 `script-src`/`style-src`。
- `<meta property="csp-nonce" nonce="...">`，供 Vite 7 的 HMR/style runtime 读取。
- 生产 stylesheet link、hydration JSON script 和 module script。
- Rust→Node renderer v2 请求的顶层 `csp_nonce`，以及 React `renderToPipeableStream` 生成的恢复脚本。

Nonce 不属于 `PageEnvelope`，不会进入页面 props、局部导航 JSON、contract hash 或共享业务缓存。包含 nonce 的 HTML/XHTML 固定覆盖为 `Cache-Control: private, no-store`，并移除 `ETag`/`Last-Modified`；JSON/API 响应的缓存策略不因 nonce 中间件改变。不要设置 Vite `html.cspNonce`，因为那会把构建期常量误用为跨请求 nonce。

静态 CSP 进入 nonce 模式前会按 ASCII 大小写不敏感规则拒绝重复 directive、`'unsafe-inline'`、预置 `'nonce-*'` 和控制字符。未显式声明 `script-src`/`style-src` 时继承 `default-src`，不会把 `default-src 'none'` 或仅 CDN 的策略放宽为 `'self'`；显式 `script-src-elem`/`style-src-elem` 也会获得相同 nonce，避免更具体 directive 覆盖基础策略。嵌套相同 nonce policy 会复用 Request 中的值；不同策略产生的下游 CSP 冲突和下游中间件 panic 都转换为带浏览器安全头的通用 500。第三方 script/style source 仍必须由应用在受审查的 `SecurityPolicy` 基础上显式允许。只有确认所有子域都支持 HTTPS 后才启用 HSTS `includeSubDomains`，HSTS 本身不能替代 TLS 终止。
...
需要按 UDID、邮箱等敏感字段做精确匹配时，可把原值用 `Encryptor` 加密保存，同时把 `BlindIndexer` 生成的确定性 HMAC-SHA256 envelope 放入唯一索引列。purpose 是索引域的一部分，必须使用稳定且不可混用的 1-128 byte 标识；Phoenix 还会分别长度编码格式版本、key ID、purpose 和 value，避免字段拼接碰撞：

```rust
use phoenix::prelude::*;

let current = BlindIndexKey::new("INDEX_2026_07", [9_u8; 32])?;
let old = BlindIndexKey::new("INDEX_2026_01", [8_u8; 32])?;
let indexer = BlindIndexer::new(current).with_verification_key(old)?;
let normalized = "00008020-001C2D1234567890";

let stored = indexer.index("APP_A|devices|udid:v1", normalized.as_bytes())?;
let query_candidates =
    indexer.candidates("APP_A|devices|udid:v1", normalized.as_bytes())?;
indexer.verify(&stored, "APP_A|devices|udid:v1", normalized.as_bytes())?;
# Ok::<(), Box<dyn std::error::Error>>(())
```

envelope 固定为 `phxbi.v1.hmac-sha256.<base64url-kid>.<base64url-tag>`。新写入只使用 active key；查询时按 active、legacy 加入顺序生成最多 `MAX_BLIND_INDEX_KEYS`（当前为 8）个候选。匹配旧候选后，应用应使用本次已知的规范化原值把该行重写为 active envelope；轮换窗口结束并完成迁移后再移除旧 key。未知 key ID、非规范 Base64URL、错误版本/算法、错误长度和认证失败都会失败关闭。

盲索引不是加密，也不隐藏相同值之间的重复关系。低熵字段在索引与 HMAC key 同时泄露后仍可被离线枚举；手机号、短邀请码、枚举状态等尤其不能把盲索引当作保密措施。每种业务字段应先定义唯一规范化策略，并使用独立的 purpose。盲索引 key 必须由密码学安全随机源生成、保存在仓库外，而且必须独立于 `EncryptionKey`、JWT、Session/Cookie、CSRF 和其他 HMAC key；不能从这些 key 派生或直接复用。示例数组只说明类型，不是生产密钥。

## 3.3 尚未实现的安全能力

- Redis 生产适配器已提供（`phoenix-redis` feature）；多密钥 Cookie session 仍未实现。Memory backend 继续只面向单进程/契约测试。
- Multipart 默认仍在内存解析；本地上传落盘与 key 净化已由 `phoenix-storage` 提供。生产前端静态资源由脚手架注册的 `ServeProductionAssets`（manifest 白名单 + `public/assets`）提供；通用任意路径静态目录与流式 multipart 落盘仍待补齐。安全下载响应已经提供。
- 正式发布前的独立安全评审仍待完成（门禁与依赖审计已有首版）。
...
- 依赖：CI 使用锁文件执行 `cargo-deny` 与 `npm audit`；许可证策略、唯一临时 RustSec 例外和删除条件见[工程质量门禁](QUALITY_GATES.md)。
