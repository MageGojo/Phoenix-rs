...
开发工具
  -> phoenix-cli（Rust/Vite 双进程监督与统一退出）
...
`phoenix-dx` 提供标准 resource routes、中间件别名、异步模型绑定和编译期 `routes/*.rs` 自动挂载。`phoenix-cli` 的 `phoenix dev` 同时启动 Rust 与 Vite，并在 Ctrl-C 或任一子进程退出时回收两个进程组。
