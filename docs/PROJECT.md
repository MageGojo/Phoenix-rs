...
`phoenix-dx` 提供标准 resource routes、中间件别名、异步模型绑定和编译期 `routes/*.rs` 自动挂载。`phoenix-cli` 负责 `phoenix new`、业务生成器和 `phoenix dev`；生成器只改写带 `<phoenix:...>` 标记的托管区块，自动注册 Rust modules、模型、迁移和 TypeScript contracts/routes。`phoenix dev` 同时启动 Rust 与 Vite，并在 Ctrl-C 或任一子进程退出时回收两个进程组。
...
| React 与后端双进程开发复杂 | 首次启动失败率高 | 使用 `phoenix dev` 统一启动、传播退出信号并回收子进程组 |
