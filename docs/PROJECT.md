...
Phoenix-rs 采用模块化单体框架结构。应用开发者通常只依赖顶层 `phoenix` crate（crates.io 包名 `phoenixrs`）；内部组件保持独立，以便测试边界和替换底层实现。框架不隐藏 Rust 类型系统，而是提供更短、更一致的常用路径。
...
应用代码
  -> phoenix / phoenixrs（统一导出与 prelude）
      ...
      -> phoenix-http（请求、响应、Cookie、上传、SSE/WebSocket）
      ...
      -> phoenix-release（发版制品、校验、原子切换、回滚）
      -> phoenix-console（应用子命令 Console）
      -> phoenix-queue / phoenix-mail（可选 feature）
      -> phoenix-redis（Session/限流/Token 的 Redis 适配器）
      -> phoenix-storage（本地上传存储与路径净化）
      -> phoenix-testing（请求、Cookie 与页面断言）
      -> phoenix-logging（进程级 tracing）

开发工具
  -> phoenix-cli 目录 / crates.io 包 `px`（脚手架、生成器、px dev、px release*）

浏览器
  -> phoenix-react / phoenix-vite / phoenix-react-ssr
```
...
负责 `config/app.toml` + `config/database.toml`、`.env`、进程环境与显式 override 的确定性合并（优先级：TOML < `.env` < 环境 < builder）。提供开发/测试/生产环境、URL/数据库/代理/Host/限流校验，以及由应用按实际用途声明的必填 secret。编辑器补全见仓库 `schemas/` 与 `taplo.toml`（详见 [CONFIG.md](CONFIG.md)）。敏感值的 Debug 输出固定脱敏，不推断或复用 JWT、加密、盲索引等不同用途的密钥。
...
- 流式响应已通过框架 `ResponseBody::Stream` 接入 Hyper。流式请求由路由显式选择 `RequestBodyMode::Streaming`，以 one-shot `RequestBodyStream` 在 handler 拉取时施加背压。
- SSE 与 HTTP/1.1 WebSocket Upgrade 已交付首版；HTTP/2 CONNECT / RFC 8441 未交付（见 [REALTIME.md](REALTIME.md)）。

当前实现使用 Hyper/hyper-util 在同一监听器自动服务 HTTP/1.1 与 HTTP/2，并允许应用限制为 `Http1Only` 或 `Http2Only`。rustls 接入层提供 PEM/文件证书、TLS 握手 deadline 和与协议策略一致的 `h2`/`http/1.1` ALPN；真实网络测试覆盖 TLS + HTTP/2。请求保留原始 HTTP version 与 extensions，供 `OnUpgrade` 与后续 H2 扩展使用。
...
CLI 已提供迁移骨架生成并自动维护应用迁移注册表；`px migrate/status/rollback/fresh/seed` 通过项目内管理二进制直接调用 Phoenix `MigrationRunner` 和应用 seeder，不依赖 Toasty CLI。

## 8. 配置与部署

- 配置来源按“默认值 < `.env` < 环境变量 < 显式 override”合并，敏感值不进入仓库。
- 开发默认 SQLite，生产文档推荐 PostgreSQL；支持矩阵由契约测试决定。
- 生产环境要求显式公开 URL、数据库 URL、可信代理和 Host allowlist；JWT、加密等 secret 由实际消费方分别声明名称和最小长度。
